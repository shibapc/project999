...
def create_excel(chat_id: int, user_data: dict):
    ...
    try:
        ...
        for sheet_name in sheets:
            ...
            if not products:
                logger.warning(f"Нет продуктов для листа {sheet_name}")
            ...
            total_row_formula = f"=SUM(E2:E{len(products) + 1})"
        ...
        for idx, sheet_name in enumerate(sheets, start=2):
            total_sum_formula = f"=SUM('{sheet_name}'!E2:E{len(user_data[chat_id]['products'].get(sheet_name, [])) + 1})"
        ...
        # Создаём директорию smeta_files, если она не существует
        os.makedirs("smeta_files", exist_ok=True)
